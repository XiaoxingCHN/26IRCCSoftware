...
/**
  ...
  */
float PID_Calculate_Position(PID_Info_TypeDef *PID, float Target,float Measure)
{		
...
* @author XMX
*/
float PID_Calculate_Increment(PID_Info_TypeDef *PID, float Target,float Measure)
{
  /* update the PID error status */
  PID_ErrorHandle(PID);
  if(PID->ERRORHandler.Status != PID_ERROR_NONE)
  ...
  if(fabsf(PID->Err[0]) >= PID->Param.Deadband)
  {
    ...
    if(PID->Type == PID_POSITION)
    {
      ...
      /* update the PID output */
      //增量式输出为当前输出与上次输出之差
      float IncrementOutput = PID->Pout + PID->Iout + PID->Dout;
      VAL_LIMIT(IncrementOutput,-PID->Param.LimitOutput,PID->Param.LimitOutput);
      PID->Output += IncrementOutput;
      VAL_LIMIT(PID->Output,-PID->Param.LimitOutput,PID->Param.LimitOutput);
    }
    else if(PID->Type == PID_VELOCITY)
    {
